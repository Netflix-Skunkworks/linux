...
#include <linux/bpf.h>
#ifndef MAX_BPRINTF_VARGARGS /* linux < 5.19 */
			       ...
			       void *kdata, const void *udata)
{
	...
	u32 moff = btf_member_bit_offset(t, member) / 8;
	int ret;
...
#ifndef for_each_cpu_andnot /* linux < 6.1 */

#ifndef find_next_andnot_bit /* linux < 6.1 */
...
static unsigned long _find_next_andnot_bit(const unsigned long *addr1, const unsigned long *addr2,
...
BTF_SET_START(scx_kfunc_ids_init)
BTF_ID(func, scx_bpf_switch_all)
BTF_SET_END(scx_kfunc_ids_init)
...
BTF_SET_START(scx_kfunc_ids_sleepable)
BTF_ID(func, scx_bpf_create_dsq)
BTF_SET_END(scx_kfunc_ids_sleepable)
...
BTF_SET_START(scx_kfunc_ids_dispatch)
BTF_ID(func, scx_bpf_dispatch_nr_slots)
BTF_ID(func, scx_bpf_dispatch)
BTF_SET_END(scx_kfunc_ids_dispatch)
...
BTF_SET_START(scx_kfunc_ids_consume)
BTF_ID(func, scx_bpf_consume)
BTF_SET_END(scx_kfunc_ids_consume)
...
BTF_SET_START(scx_kfunc_ids_cpu_release)
BTF_ID(func, scx_bpf_reenqueue_local)
BTF_SET_END(scx_kfunc_ids_cpu_release)
...
BTF_SET_START(scx_kfunc_ids_online)
BTF_ID(func, scx_bpf_kick_cpu)
BTF_ID(func, scx_bpf_dsq_nr_queued)
BTF_ID(func, scx_bpf_test_and_clear_cpu_idle)
BTF_ID(func, scx_bpf_pick_idle_cpu)
BTF_SET_END(scx_kfunc_ids_online)
...
BTF_SET_START(scx_kfunc_ids_any)
BTF_ID(func, scx_bpf_error_bstr)
BTF_ID(func, scx_bpf_destroy_dsq)
BTF_ID(func, scx_bpf_task_running)
BTF_ID(func, scx_bpf_task_cpu)
BTF_SET_END(scx_kfunc_ids_any)

static bool bpf_scx_btf_check_kfunc_call(u32 kfunc_btf_id)
{
	return  btf_id_set_contains(&scx_kfunc_ids_init, kfunc_btf_id) ||
		btf_id_set_contains(&scx_kfunc_ids_sleepable, kfunc_btf_id) ||
		btf_id_set_contains(&scx_kfunc_ids_dispatch, kfunc_btf_id) ||
		btf_id_set_contains(&scx_kfunc_ids_consume, kfunc_btf_id) ||
		btf_id_set_contains(&scx_kfunc_ids_cpu_release, kfunc_btf_id) ||
		btf_id_set_contains(&scx_kfunc_ids_online, kfunc_btf_id) ||
		btf_id_set_contains(&scx_kfunc_ids_any, kfunc_btf_id);
}
...
const struct bpf_verifier_ops bpf_scx_verifier_ops = {
	...
	.check_kfunc_call = bpf_scx_btf_check_kfunc_call,
};
